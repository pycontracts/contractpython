...
void bind_ConstructorStats(py::module &m)
{
    ...
    // Not exactly ConstructorStats, but related: expose the internal pybind number of registered
    // instances to allow instance cleanup checks (invokes a GC first)
...
PYBIND11_MODULE(pybind11_tests, m)
{
    ...
    py::class_<IncType, UserType>(m, "IncType").def(py::init<>()).def(py::init<int>()).def("__repr__", [](const IncType &u) {
