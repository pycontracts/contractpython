...
template <typename T> std::shared_ptr<T> wrap_value(T *val) { return std::shared_ptr<T>{ val }; }
...
public:
    ...
    ValuePtr duplicate() { return duplicate(memory_manager()); }

    virtual std::string str() const { throw std::runtime_error("Cannot convert value to string"); }

    virtual uint32_t size() const { throw std::runtime_error("Cannot get size of value"); }
    ...
    virtual bool can_iterate() const { return false; }

    virtual bool is_callable() const { return false; }

    virtual bool bool_test() const { return true; }
...
{
public:
    CustomValue(MemoryManager &mem) : cow::Value(mem) {}

    ValueType type() const override { return ValueType::Custom; }
...
template <typename value_type, ValueType value_type_val> class PlainValue : public Value
{
public:
    PlainValue(MemoryManager &mem, value_type val) : Value(mem), m_value(val) {}

    ValueType type() const override { return value_type_val; }

    const value_type &get() const { return m_value; }

    void set(const value_type &v) { m_value = v; }

    std::string str() const override { return std::to_string(m_value); }
...
public:
    ...
    ValueType type() const override { return ValueType::Alias; }
...
{
public:
    BoolVal(MemoryManager &mem, bool val) : PlainValue(mem, val) {}
...
{
public:
    StringVal(MemoryManager &mem, const std::string &val) : Value(mem), m_value(val) {}
    ...
    std::string str() const override { return get(); }

    uint32_t size() const override { return m_value.size(); }
...
{
public:
    FloatVal(MemoryManager &mem, const double &val) : PlainValue(mem, val) {}
...
{
public:
    IntVal(MemoryManager &mem, const int64_t &val) : PlainValue(mem, val) {}
...
{
public:
    value_exception(const std::string &what) : std::exception(), m_what(what) {}
...
template <typename T> std::shared_ptr<T> value_cast(ValuePtr val)
{
...
inline bool operator==(const Value &first, const Value &second)
{
    ...
    {
        return dynamic_cast<const StringVal &>(first).get() ==
               dynamic_cast<const StringVal &>(second).get();
    }
